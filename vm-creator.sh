...
--full-user-name=$fullUname --user=$Uname \ 
--password=$Password --locale=$Locale \ 
--country=$Country --time-zone=$Zone --hostname=$Fqdn \ 
--iso=$osISO --install-additions \ 
--additions-iso="/home/`whoami`/.config/VirtualBox/VBoxGuestAdditions_6.1.8.iso"  \ 
--start-vm=$startMode
--post-install-command=$pInstall

echo -e "Checking if $VMName is running"
VBoxManage list runningvms | grep $VMName

echo -e "\n [+] Wait for installation to finish \n"
#VBoxManage guestproperty wait $VMName

echo -e "Post Installation Actions"
